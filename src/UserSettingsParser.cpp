...
/**
 * @brief Retrieves the path to the user configuration file.
 *
 * @throws std::runtime_error If the file cannot be opened.
 */
const char* UserSettingsParser::getUsrConfigPath(void)
{
    std::lock_guard<std::mutex> lock(mutex_);
    std::ifstream file(SECURE_MOUNT_FLAG_FILE);
    if (!file.is_open()) {
        return nullptr;
    }
    std::string line;
    ...
    file.close();
    if (strcasecmp(line.c_str(), "SUCCESS") == 0) {
        return SECURE_USR_CONFIG_FILE_PATH;
    } else if (strcasecmp(line.c_str(), "FAIL") == 0)
    {
        return USR_CONFIG_FILE_PATH;
    }
    else
    {
        return nullptr;
...
/**
 * @brief Retrieves the path to the WIFI configuration file.
 *
 * @throws std::runtime_error If the file cannot be opened.
 */
const char* UserSettingsParser::getWifiConfigPath(void)
{
    std::lock_guard<std::mutex> lock(mutex_);
    std::ifstream file(SECURE_MOUNT_FLAG_FILE);
    if (!file.is_open()) {
    ...
    file.close();
    if (strcasecmp(line.c_str(), "SUCCESS") == 0) {
        return SECURE_WPA_CONFIG_FILE_PATH;
    } else if (strcasecmp(line.c_str(), "FAIL") == 0)
    {
        return WPA_CONFIG_FILE_PATH;
    }
    else
    {
        return nullptr;
